...
function cle() {
  keyFounded = true;
  localStorage.setItem("cletrouve",true);
  goToChapter('inconnue');
};

function affaible() {
  faible = true;
  localStorage.setItem("faibletrouve",true);
  goToChapter('inconnue');
};
...
function goToChapter(chapterName) {
  ...
  localStorage.setItem("chapterplace",chapterName);

let chaptersubtitle = chaptersObj[chapterName]["subtitle"];
let chaptertexte = chaptersObj[chapterName]["text"];
let chapterimage = chaptersObj[chapterName]["image"];
let chapteroptions = chaptersObj[chapterName]["options"];
let videooptions = chaptersObj[chapterName]["video"];
let audiooptions = chaptersObj[chapterName]["audio"];
...
HTMLaudio.src = audiooptions;
      ...
      let body = document.querySelector("body");
       if (chapterName !== "mission") {
    body.classList.remove("mission");
    }else{
      body.classList.add("mission");
    };
 
   if (chapterName !== "flashback") {
...
if ('video' in chaptersObj[chapterName]) {
  ...
  let checkboxson = document.getElementById("son");
  localStorage.setItem("chapterson",checkboxson.checked);
  if (checkboxson.checked == true) {
    HTMLaudio.play();
    }else{
    HTMLaudio.pause();  
    };
...
let savegame = localStorage.getItem("chapterplace");
if (savegame==null) {
  goToChapter("reveil");
  console.log("reveil");
  }else{
...
let savecle = localStorage.getItem("cletrouve");
let savefaible = localStorage.getItem("faibletrouve");
if (savecle === true) {
  keyFounded=Boolean(savecle);
  }else if (savefaible === true){ 
    faible=Boolean(savefaible);
  }else{
    keyFounded = false;
    faible = false; 
  }
  ...
  let joueurson = true;
  let saveson = localStorage.getItem("chapterson");
  if (saveson == true) {
    joueurson = true;
    }else{
      joueurson = false;
